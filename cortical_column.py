...
class TwoDimCorticalColumn:
    ...
    def __init__(self, t_rc, t_ref, t_psc, c_spacing, m, m_prev, i, prev_c_positions,
        ...
        # Layer 6 relative shift from previous to current level
        # D0: shift in x direction
        # D1: shift in y direction
        self.l6_shift = nengo.Ensemble(
            300,  # population size
            2,    # dimensionality
            max_rates=nengo.dists.Uniform(100, 200),
    ...
    def l6_relative_shift(self, l5_pos_out):
        tgt_pos_x, tgt_pos_y, theta_curr_x, theta_curr_y = l5_pos_out

        if abs(tgt_pos_x) <= self.m_prev:
            theta_prev_x = 0
        else:
            theta_prev_x = tgt_pos_x - np.sign(tgt_pos_x) * self.m_prev

        if abs(tgt_pos_y) <= self.m_prev:
            theta_prev_y = 0
        else:
            theta_prev_y = tgt_pos_y - np.sign(tgt_pos_y) * self.m_prev

        s_x = theta_prev_x - theta_curr_x
        s_y = theta_prev_y - theta_curr_y

        return s_x, s_y

    def l6_sampling_factor(self, tgt_size):
        return (min(max(tgt_size*self.size_max, self.size_curr_l[0]), self.size_prev_l[0]) - 1) / \
             (self.size_curr_l[0] - 1)
