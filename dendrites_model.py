# -*- coding: utf-8 -*-
"""
Model Laminar layer 4 of a cortical column
"""
...
class Layer4:
    ...
    def __init__(self, t_rc, t_ref, t_psc, c_spacing, j, i, m,
        ...
        for e_idx, ens in enumerate(self.dend.ensembles):
            ...
            nengo.Connection(self.dend_rout, ens[0], eval_points=eval_points,
                             function=self.routing_function, synapse=t_psc)

            # Dendrites ---> soma
            nengo.Connection(ens, self.soma.neurons[e_idx],
                             synapse=t_psc,  # eval_points=eval_points2,
                             function=self.scaled_dend_out, transform=encoders[e_idx])
...
if __name__ == '__main__':
    ...
    prev_level_column_pos = np.array([-0.25, 0, 0.25])
    ...
    for l4_idx, l4 in enumerate(l4_arr):
        f1, ax_arr = plt.subplots(2, 1)
        f1.suptitle("Column pos %0.2f, previous layer connected pos %0.2f"
                    % (column_pos, prev_level_column_pos[l4_idx]))
